...
{
   printf("[STUB] [graphics] visible_bitmap_rect: argcnt=%i x=%i y=%i flip=%u table=%u number=%u array=%p\n", argcnt, x, y, flip, table, number, (void *)array);
   return 0;
}
...
{
   printf("[STUB] [graphics] vsprint: argcnt=%i wndnum=%u format=%p argprt=%p\n", argcnt, wndnum, (void *)format, (void *)argptr);
}
